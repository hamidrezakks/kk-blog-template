		...
		|| (isset($_SESSION["adminbykk"]) && $_SESSION["adminbykk"][3] == "10") && isset($_POST["login"]) && $_POST["login"] == '1')
{
	...
	$query = mysql_query("SELECT id, email, password, name, country, md5_code, user_level, uadmin, state FROM registerbykk WHERE email = '$lemail' AND ulaerror < 10 AND state != 0 ;",$db);
	if($query)
	{
		if($lrow = mysql_fetch_row($query))
		{
	...
	{
		$_SESSION["login_msg"][0] = "0";
		$_SESSION["login_msg"][1] = "Email or password is wrong!". mysql_error();
	}
}
